...
(
    ...
    where dt >= date_sub(current_date, 15)
        and checkout_date between '2026-01-15' and '2026-01-23'
...
(
    select distinct `日期`
     ...
     left join(
        ...
        where dt >= '2025-11-01'
            and substr(create_time, 1, 10) >= '2025-11-01'  -- 生单时间
...
(
    ...
    left join (
        ...
        where dt >= date_sub(current_date, 50)
...
,q_order as (----订单明细表表包含取消  分目的地、新老维度
    ...
    where dt = '%(DATE)s'
        ...
        and checkout_date between '2026-01-15' and '2026-01-23'   --- 离店日期在国庆
...
select t1.`日期`,t1.`渠道`,t1.mdd,`流量占比`,UV
        ,concat(round(nvl(`订单量` / UV,0) * 100,2),'%') as `CR`
        ,concat(round(nvl(`Q_用券订单量`/ `订单量` ,0) * 100,2),'%') as `用券订单占比`
        ,`订单量`,`间夜量`
        ,`新客UV`,`新客流量占比`
...
(
    select dates as `日期`
            ...
            ,UV
            ,`新客UV`
            ,concat(round(UV / sum(UV) over(partition by dates,mdd) * 100,2), '%') `流量占比`
            ,concat(round(`新客UV` / sum(`新客UV`) over(partition by dates) * 100,2), '%') `新客流量占比`
    from q_uv_info
)t1 left join (
    select order_date as `日期`
            ...
            ,`Q_新客用券订单量` 
    from order_info
)t2 on t1.`日期`=t2.`日期` and t1.`渠道`=t2.`渠道` and t1.mdd=t2.mdd
order by  t1.`渠道`,t1.`日期` desc,t1.mdd desc
;