...
,q_order as ( --- Q侧订单明细打标
    ...
    where a.dt = from_unixtime(unix_timestamp() -86400, 'yyyyMMdd')
        ...
        and a.order_date >= '2026-05-01' and a.order_date <= date_sub(current_date, 1)
...
,c_order as ( 
    ...
    left join (
        ...
        where dt = '%(FORMAT_DATE)s'
            ...
            and substr(order_date,1,10) >= '2026-05-01' and substr(order_date,1,10) <= cast(date_sub(current_date, 1) as string)
...
,q_agg as (
    ...
    group by order_date,per_type,mdd,user_type,adr_type,supplier_type,linqi_type
    grouping sets (
        (order_date,per_type,mdd,user_type),  -- 核心维度：目的地&提前订&新老客
        (order_date,per_type,mdd,adr_type),   -- 核心维度：目的地&提前订&价格带
        (order_date,per_type,mdd,supplier_type), -- 核心维度：目的地&提前订&货源
        (order_date,per_type,user_type),  -- 核心维度：提前订&新老客
        (order_date,per_type,adr_type),   -- 核心维度：提前订&价格带
        (order_date,per_type,supplier_type), -- 核心维度：提前订&货源
        (order_date,mdd,user_type),  -- 核心维度：目的地&新老客
        (order_date,mdd,adr_type),   -- 核心维度：目的地&价格带
        (order_date,mdd,supplier_type), -- 核心维度：目的地&货源
        (order_date,per_type), -- 核心维度：提前订
        (order_date,mdd), -- 核心维度：目的地

        -- 【新增】基于 linqi_type 的分组，保持同等维度下钻
        (order_date,linqi_type,mdd,user_type),  -- 核心维度：目的地&提前订&新老客
        (order_date,linqi_type,mdd,adr_type),   -- 核心维度：目的地&提前订&价格带
        (order_date,linqi_type,mdd,supplier_type), -- 核心维度：目的地&提前订&货源
        (order_date,linqi_type,user_type),  -- 核心维度：提前订&新老客
        (order_date,linqi_type,adr_type),   -- 核心维度：提前订&价格带
        (order_date,linqi_type,supplier_type), -- 核心维度：提前订&货源
        (order_date,linqi_type), -- 核心维度：提前订
        (order_date)
    )
)
,c_agg as (
    ...
    from c_order
    group by dt,per_type,mdd,user_type,adr_type,supplier_type,linqi_type
    grouping sets (
        (dt,per_type,mdd,user_type),  -- 核心维度：目的地&提前订&新老客
        (dt,per_type,mdd,adr_type),   -- 核心维度：目的地&提前订&价格带
        (dt,per_type,mdd,supplier_type), -- 核心维度：目的地&提前订&货源
        (dt,per_type,user_type),  -- 核心维度：提前订&新老客
        (dt,per_type,adr_type),   -- 核心维度：提前订&价格带
        (dt,per_type,supplier_type), -- 核心维度：提前订&货源
        (dt,mdd,user_type),  -- 核心维度：目的地&新老客
        (dt,mdd,adr_type),   -- 核心维度：目的地&价格带
        (dt,mdd,supplier_type), -- 核心维度：目的地&货源
        (dt,per_type), -- 核心维度：提前订
        (dt,mdd), -- 核心维度：目的地

        -- 【新增】基于 linqi_type 的分组，保持同等维度下钻
        (dt,linqi_type,mdd,user_type),  -- 核心维度：目的地&提前订&新老客
        (dt,linqi_type,mdd,adr_type),   -- 核心维度：目的地&提前订&价格带
        (dt,linqi_type,mdd,supplier_type), -- 核心维度：目的地&提前订&货源
        (dt,linqi_type,user_type),  -- 核心维度：提前订&新老客
        (dt,linqi_type,adr_type),   -- 核心维度：提前订&价格带
        (dt,linqi_type,supplier_type), -- 核心维度：提前订&货源
        (dt,linqi_type), -- 核心维度：提前订
        (dt)
    )
)
...
from q_agg q
left  join c_agg c on q.dt = c.dt and q.per_type = c.per_type and q.mdd = c.mdd and q.adr_type = c.adr_type and q.supplier_type = c.supplier_type and q.user_type=c.user_type
order by `日期` desc;
;
...
,q_traf as (
    ...
    left join temp.temp_yiquny_zhang_ihotel_area_region_forever e on a.country_name = e.country_name 
    where a.dt >= '2026-05-01' and a.dt <= date_sub(current_date, 1)
...
,c_order as (
    ...
    where dt = cast(date_sub(current_date, 1) as string)
      ...
      and (extend_info['CANCEL_TIME'] is null or extend_info['CANCEL_TIME']='NULL' or substr((extend_info['CANCEL_TIME']),1,10)>substr(order_date,1,10))
      and substr(o.order_date,1,10) >= '2026-05-01' and substr(o.order_date,1,10) <= date_sub(current_date, 1)
...
,q_uv_agg as (
    ...
    from q_traf t
    group by t.dt, t.new_mdd, t.user_type, t.per_type, t.linqi_type
    grouping sets (
        (t.dt),
        (t.dt,t.new_mdd),
        (t.dt,t.per_type),
        (t.dt,t.per_type,t.user_type),
        (t.dt,t.per_type,t.new_mdd),
        -- 新增：临期大维度的聚合组合
        (t.dt,t.linqi_type),
        (t.dt,t.linqi_type,t.user_type),
        (t.dt,t.linqi_type,t.new_mdd)
    )
)
,q_ord_agg as (
    select t.order_date as mth
          ,case when grouping(per_type)=0 then per_type 
                when grouping(linqi_type)=0 then linqi_type
                else 'ALL' 
           end as per_type
    ...
    from q_order t
    group by t.order_date,t.new_mdd, t.user_type, t.per_type, t.linqi_type
    grouping sets (
        (t.order_date),
        (t.order_date,t.new_mdd),
        (t.order_date,t.per_type),
        (t.order_date,t.per_type,t.new_mdd),
        (t.order_date,t.per_type,t.user_type),
        -- 新增：临期大维度的聚合组合
        (t.order_date,t.linqi_type),
        (t.order_date,t.linqi_type,t.user_type),
        (t.order_date,t.linqi_type,t.new_mdd)
    )
)
,c_uv_agg as (
    select t.dt as mth
...
select coalesce(q.mth, c.mth) as `日期`
      ,coalesce(q.per_type, c.per_type) as `提前订类型`
...
from q_uv_agg q
left join c_uv_agg c on q.mth = c.mth  and q.user_type = c.user_type and q.per_type = c.per_type and q.new_mdd = c.new_mdd
left join q_ord_agg qo on q.mth = qo.mth  and q.user_type = qo.user_type and q.per_type = qo.per_type and q.new_mdd = qo.new_mdd
left join c_order_agg co on c.mth = co.mth  and c.user_type = co.user_type and c.per_type = co.per_type and c.new_mdd = co.new_mdd
...
,q_order as ( --- Q侧订单明细打标
    select a.order_date
          ...
          -- 4. 不可取消判定
          ,case when cast(split(get_json_object(extendInfoMap, '$.homogenizationKey'),'\\|')[2] as int) = 0 then 'Y' else 'N' end as is_non_ref 
          ,a.user_id,a.order_no, a.final_gmv, a.room_night
          ,case when (batch_series like '%23base_ZK_728810%' or batch_series like '%23extra_ZK_ce6f99%')
    ...
    where a.dt = from_unixtime(unix_timestamp() -86400, 'yyyyMMdd')
        ...
        and is_valid='1'
        and a.order_no <> '103576132435'
        and a.checkout_date >= '2026-05-01' and a.checkout_date <= date_sub(current_date, 1)
        and province_name in ('澳门','香港') -- 只分析港澳订单
)
,c_order as ( 
    select substr(o.order_date,1,10) as dt
    ...
    where dt = cast(date_sub(current_date, 1) as string)
      ...
      and terminal_channel_type = 'app' 
      and order_status <> 'C'
      and substr(checkout_date,1,10) >= '2026-05-01' and substr(checkout_date,1,10) <= cast(date_sub(current_date, 1) as string)
      and extend_info['PROVINCE'] in ('澳门','香港')
)
,q_agg as (
    ...
    group by checkout_date,per_type,mdd,user_type,adr_type,supplier_type,linqi_type
    grouping sets (
        (checkout_date,per_type,mdd,user_type),  -- 核心维度：目的地&提前订&新老客
        (checkout_date,per_type,mdd,adr_type),   -- 核心维度：目的地&提前订&价格带
        (checkout_date,per_type,mdd,supplier_type), -- 核心维度：目的地&提前订&货源
        (checkout_date,per_type,user_type),  -- 核心维度：提前订&新老客
        (checkout_date,per_type,adr_type),   -- 核心维度：提前订&价格带
        (checkout_date,per_type,supplier_type), -- 核心维度：提前订&货源
        (checkout_date,mdd,user_type),  -- 核心维度：目的地&新老客
        (checkout_date,mdd,adr_type),   -- 核心维度：目的地&价格带
        (checkout_date,mdd,supplier_type), -- 核心维度：目的地&货源
        (checkout_date,per_type), -- 核心维度：提前订
        (checkout_date,mdd), -- 核心维度：目的地

        -- 【新增】基于 linqi_type 的分组，保持同等维度下钻
        (checkout_date,linqi_type,mdd,user_type),  -- 核心维度：目的地&提前订&新老客
        (checkout_date,linqi_type,mdd,adr_type),   -- 核心维度：目的地&提前订&价格带
        (checkout_date,linqi_type,mdd,supplier_type), -- 核心维度：目的地&提前订&货源
        (checkout_date,linqi_type,user_type),  -- 核心维度：提前订&新老客
        (checkout_date,linqi_type,adr_type),   -- 核心维度：提前订&价格带
        (checkout_date,linqi_type,supplier_type), -- 核心维度：提前订&货源
        (checkout_date,linqi_type), -- 核心维度：提前订
        (checkout_date)
    )
)
,c_agg as (
...
--- 4、离店取消率数据
with q_detail as (--- Q取消率明细
    select substr(a.checkout_date, 1, 10) as checkout_date
        ,case when a.province_name in ('澳门', '香港') then a.province_name
              else '其他'
         end as new_mdd
        ,case when datediff(a.checkin_date, a.order_date) <= 0 then '凌晨订&当天订'
              when datediff(a.checkin_date, a.order_date) between 1 and 3 then '提前订1-3天'
              when datediff(a.checkin_date, a.order_date) between 4 and 7 then '提前订4-7天'
              when datediff(a.checkin_date, a.order_date) between 8 and 14 then '提前订8-14天'
              else '提前订15+'
         end as per_type
        ,case when datediff(a.checkin_date, a.order_date) <= 7 then '临期订'
              else '非临期订'
         end as linqi_type
        ,a.order_no
        ,a.room_night
    ...
    where a.dt = from_unixtime(unix_timestamp() - 86400, 'yyyyMMdd')
        ...
        and (a.refund_time is null or date(a.refund_time) > a.order_date) -- 非当天取消&拒单
        and a.is_valid = '1'
        and substr(a.checkout_date, 1, 10) between '2026-05-01' and date_sub(current_date, 1)
        and a.order_no <> '103576132435'
        and a.province_name in ('澳门', '香港') -- 只分析港澳订单
)
,c_detail as (--- C取消率明细
    select substr(o.checkout_date, 1, 10) as checkout_date
        ,case when o.extend_info['PROVINCE'] in ('澳门', '香港') then o.extend_info['PROVINCE']
              else '其他'
         end as new_mdd
        ,case when datediff(substr(o.checkin_date, 1, 10), substr(o.order_date, 1, 10)) <= 0 then '凌晨订&当天订'
              when datediff(substr(o.checkin_date, 1, 10), substr(o.order_date, 1, 10)) between 1 and 3 then '提前订1-3天'
              when datediff(substr(o.checkin_date, 1, 10), substr(o.order_date, 1, 10)) between 4 and 7 then '提前订4-7天'
              when datediff(substr(o.checkin_date, 1, 10), substr(o.order_date, 1, 10)) between 8 and 14 then '提前订8-14天'
              else '提前订15+'
         end as per_type
        ,case when datediff(substr(o.checkin_date, 1, 10), substr(o.order_date, 1, 10)) <= 7 then '临期订'
              else '非临期订'
         end as linqi_type
        ,o.order_no
        ,cast(o.extend_info['room_night'] as bigint) as room_night
        ,o.order_status
    from ihotel_default.ceq_three_sync_pull_edw_trade_tripart_qunar_oversea_hotelorder_reconfig_da o
    where o.dt = '%(FORMAT_DATE)s'
        ...
        and (o.extend_info['CANCEL_TIME'] is null
            or o.extend_info['CANCEL_TIME'] = 'NULL'
            or substr(o.extend_info['CANCEL_TIME'], 1, 10) > substr(o.order_date, 1, 10))
        and substr(o.checkout_date, 1, 10) between '2026-05-01' and date_sub(current_date, 1)
        and o.extend_info['PROVINCE'] in ('澳门', '香港') -- 只分析港澳订单
)
,q_base as (--- Q按目的地、提前订维度聚合
    select checkout_date
        ,if(grouping(new_mdd) = 1, '港澳', new_mdd) as new_mdd
        ,case when grouping(per_type) = 0 then per_type
              when grouping(linqi_type) = 0 then linqi_type
              else 'ALL'
         end as per_type
        ,sum(room_night) as q_rn
        ,sum(case when order_status in ('CANCELLED', 'REJECTED') then room_night else 0 end) as q_rn_cancel
        ,count(distinct order_no) as q_order
        ,count(distinct case when order_status in ('CANCELLED', 'REJECTED') then order_no end) as q_order_cancel
    from q_detail
    ...
    grouping sets (
        (checkout_date),
        (checkout_date, new_mdd),
        (checkout_date, per_type),
        (checkout_date, per_type, new_mdd),
        (checkout_date, linqi_type),
        (checkout_date, linqi_type, new_mdd)
    )
)
,c_base as (--- C按目的地、提前订维度聚合
    select checkout_date
        ,if(grouping(new_mdd) = 1, '港澳', new_mdd) as new_mdd
        ,case when grouping(per_type) = 0 then per_type
              when grouping(linqi_type) = 0 then linqi_type
        ...
        ,sum(room_night) as c_rn
        ,sum(case when order_status = 'C' then room_night else 0 end) as c_rn_cancel
        ,count(distinct order_no) as c_order
        ,count(distinct case when order_status = 'C' then order_no end) as c_order_cancel
    from c_detail
    group by checkout_date
        ,new_mdd
        ,per_type
...
,metric_base as (--- 计算Q、C两种口径取消率
    select coalesce(t1.checkout_date, t2.checkout_date) as checkout_date
        ,coalesce(t1.new_mdd, t2.new_mdd) as new_mdd
        ,coalesce(t1.per_type, t2.per_type) as per_type
        ,t1.q_rn
        ,t1.q_rn_cancel
        ,t2.c_rn
        ,t2.c_rn_cancel
        ...
        ,t2.c_order
        ,t2.c_order_cancel
        ,t1.q_rn_cancel * 1.0000 / nullif(t1.q_rn, 0) as q_cancel_rate_rn
        ,t2.c_rn_cancel * 1.0000 / nullif(t2.c_rn, 0) as c_cancel_rate_rn
        ,t1.q_order_cancel * 1.0000 / nullif(t1.q_order, 0) as q_cancel_rate_order
        ,t2.c_order_cancel * 1.0000 / nullif(t2.c_order, 0) as c_cancel_rate_order
    from q_base t1
    full outer join c_base t2
        on t1.checkout_date = t2.checkout_date
        and t1.new_mdd = t2.new_mdd
        and t1.per_type = t2.per_type
)
,qc_base as (--- 计算间夜与订单口径取消率QC
    select checkout_date
        ,new_mdd
        ,per_type
        ,q_rn
        ...
        ,q_cancel_rate_order
        ,c_cancel_rate_order
        ,q_cancel_rate_rn / nullif(c_cancel_rate_rn, 0) as cancel_rate_qc_rn
        ,q_cancel_rate_order / nullif(c_cancel_rate_order, 0) as cancel_rate_qc_order
    from metric_base
)
select checkout_date as `离店日期`
    ,new_mdd as `目的地`
    ,per_type as `提前订类型`
    ,coalesce(q_rn, 0) as `Q间夜量`
    ,coalesce(q_rn_cancel, 0) as `Q取消间夜量`
    ,coalesce(c_rn, 0) as `C间夜量`
    ,coalesce(c_rn_cancel, 0) as `C取消间夜量`
    ,concat(round(q_cancel_rate_rn * 100, 2), '%') as `Q间夜取消率`
    ,concat(round(c_cancel_rate_rn * 100, 2), '%') as `C间夜取消率`
    ,concat(round(cancel_rate_qc_rn * 100, 2), '%') as `间夜口径取消率QC`
    ,coalesce(q_order, 0) as `Q订单量`
    ,coalesce(q_order_cancel, 0) as `Q取消订单量`
    ,coalesce(c_order, 0) as `C订单量`
    ,coalesce(c_order_cancel, 0) as `C取消订单量`
    ,concat(round(q_cancel_rate_order * 100, 2), '%') as `Q订单取消率`
    ,concat(round(c_cancel_rate_order * 100, 2), '%') as `C订单取消率`
    ,concat(round(cancel_rate_qc_order * 100, 2), '%') as `订单口径取消率QC`
    ,concat(round((cancel_rate_qc_rn - cancel_rate_qc_order) * 100, 2), 'pp') as `取消率QC差`
from qc_base
order by checkout_date desc
    ,case when new_mdd = '港澳' then 1
          when new_mdd = '香港' then 2
          when new_mdd = '澳门' then 3
          else 4
     end
    ,case when per_type = 'ALL' then 1
          when per_type = '临期订' then 2
          when per_type = '凌晨订&当天订' then 3
          when per_type = '提前订1-3天' then 4
          when per_type = '提前订4-7天' then 5
          when per_type = '非临期订' then 6
          when per_type = '提前订8-14天' then 7
          when per_type = '提前订15+' then 8
          else 9
     end
;
...
with qc_price as (
    ...
    from (
        ...
        where dt >= '20260501' and dt <= replace(date_sub(current_date, 1),'-','')
    ...
    group by order_date,per_type,channel_type,linqi_type,mdd
    grouping sets (
        (order_date),
        (order_date, mdd),
        (order_date, per_type),
        (order_date, per_type,mdd),
        (order_date, per_type,channel_type),
        (order_date, per_type,channel_type,mdd),

        (order_date, linqi_type),
        (order_date, linqi_type,mdd),
        (order_date, linqi_type,channel_type),
        (order_date, linqi_type,channel_type,mdd)
    )
)

select order_date,per_type,mdd
      ,channel_type
      ,`支付价lose率`
...
,search_child as (
    ...
    from (
        ...
        where dt >= '20260501' and dt <= '20260719'
        ...
        from ihotel_default.dw_user_app_log_detail_visit_di_v1 a 
        where dt <= '2026-07-19' and dt >= '2026-05-01'        
...
,order_result as (
    select user_id, order_date,order_date as mth,linqi_type,per_type,new_mdd
           ,order_no   -- 订单号
           ,gender     -- 性别
           ,city_level -- 城市等级
    ...
    from (
        select o.order_no
            ,o.user_id,o.order_date,linqi_type,per_type,new_mdd
            ,gender
...
,qc_room_mapping as (
    ...
    where dt >= '2026-05-01' and dt <= date_sub(current_date, 1)
    group by 1,2,3
)
,is_agent_mapping as (
    select 
    ...
    where concat(substr(a.d,1,4),'-',substr(a.d,5,2),'-',substr(a.d,7,2)) >= '2026-05-01'
...
from (
    ...
    from (
        ...
        group by a.dt,cube(supplier,linqi_type,mdd)
    ) a
) a
left join(
    ...
    from(
        ...
        from(
            ...
            left join temp.temp_yiquny_zhang_ihotel_area_region_forever e on a.country_name = e.country_name
            where  concat(substr(a.dt,1,4),'-',substr(a.dt,5,2),'-',substr(a.dt,7,2)) >= '2026-05-01'
        ...
        group by concat(substr(a.dt,1,4),'-',substr(a.dt,5,2),'-',substr(a.dt,7,2)),cube(supplier,linqi_type,mdd)
    )a
