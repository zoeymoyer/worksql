...
select t1.dt,dau,dau1,dau2 
from (
    select dt,count(user_id) dau 
    from (
        select distinct dt,mdd,user_id
        from q_uv
    ) group by 1
) t1 left join (
    select dt
           ,count(case when fromforlog in ('200000119','200000120','4104','4106','200000081','200000123','200000121','200000122','200000105','200000118','200000083','671','96','4626','913','914','4604','824')  then user_id end) dau1
           ,count(case when fromforlog in ('671','96','913','914')  then user_id end) dau2 
    from q_uv
     group by 1 
) t2 on t1.dt=t2.dt 
order by 1
...
select dt,sum(fromforlog) /  count(user_id) 
from (
select  dt ,a.user_id
        ,count(distinct fromforlog) fromforlog
from ihotel_default.mdw_user_app_log_sdbo_di_v1 a 
left join temp.temp_yiquny_zhang_ihotel_area_region_forever e on a.country_name = e.country_name 
left join user_type b on a.user_id = b.user_id 
where dt >= '2025-01-01'
