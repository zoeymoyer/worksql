...
(
    ...
    where dt >= '${zdt.addDay(-16).format("yyyy-MM-dd")}'  and dt <= '${zdt.addDay(-1).format("yyyy-MM-dd")}' 
...
(
    ...
    left join(--- 需要修改时间
        ...
        where dt >= '${zdt.addDay(-8).format("yyyy-MM-dd")}'  and dt <= '${zdt.addDay(-1).format("yyyy-MM-dd")}' 
...
(
    ...
    left join (
        ...
        where dt >= date_sub('${zdt.addDay(-1).format("yyyy-MM-dd")}' , 8)
...
select t1.dates  dt
        ...
        ,concat(round(coalesce(t2.`Q_间夜量` / t5.`Q_间夜量`, 0) * 100, 2), '%')   as `Q_间夜占比`
        ,concat(round(coalesce(t2.`Q_订单量` / t5.`Q_订单量`, 0) * 100, 2), '%')   as `Q_订单量占比`
        ,concat(round(coalesce(t2.`Q_GMV` /   t5.`Q_GMV`, 0) * 100, 2), '%')   as `Q_GMV占比`
        ,concat(round(coalesce(t2.`Q_佣金` /   t5.`Q_佣金`, 0) * 100, 2), '%')   as `Q_佣金占比`
        ,concat(round(coalesce(t2.`Q_券额` /   t5.`Q_券额`, 0) * 100, 2), '%')   as `Q_券额占比`
...
left join order_info_app t4 on t1.dates=t4.order_date and t1.mdd=t4.mdd 
and t1.user_type=t4.user_type and t1.channel=t4.channel
left join (  --- 计算流量占比
    select dates,uv
    from q_uv_info 
    where user_type = 'ALL' and channel = 'ALL' and mdd='ALL'
) t3 on t1.dates=t3.dates 
left join (  --- 计算订单占比
    select order_date,`Q_佣金`,`Q_GMV`,`Q_订单量`,`Q_券额`,`Q_间夜量`
    from order_info 
    where user_type = 'ALL' and channel = 'ALL' and mdd='ALL'
) t5 on t1.dates=t5.order_date 
order by dt ,case when mdd = '香港'  then 1
           when mdd = '澳门'  then 2
           when mdd = '泰国'  then 3
           when mdd = '日本'  then 4
           when mdd = '韩国'  then 5
