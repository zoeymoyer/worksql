...
,q_data_info as (
    ...
    left join (  --- 计算订单占比 全端
        select order_date,`Q_佣金`,`Q_GMV`,`Q_订单量`,`Q_券额`,`Q_间夜量`
        from order_info
        where user_type = 'ALL' and mdd='ALL'
...
--            ,count(distinct user_id)  `C_下单用户_ld`
...
select t1.dt
        ...
        ,`C_单间夜`
        ----- 离店口径 ------
        -- ,concat(round(`Q_间夜量_app_ld` / `C_间夜量_ld` *100, 2), '%') as `间夜QC_ld`
        -- ,concat(round(`Q_佣金_app_ld` / `C_佣金_ld` *100, 2), '%') as `收益QC_ld`
        -- ,concat(round((`Q_收益率_app_ld` - `C_收益率_ld`) *100, 2), '%')  as `收益率QC差_ld`
        -- ,concat(round(`Q_ADR_app_ld` / `C_ADR_ld` *100, 2), '%')   as `ADR_QC_ld`
        -- ,concat(round(`Q_订单量_app_ld` / `C_订单量_ld` *100, 2), '%')  as `订单量QC_ld`
        -- ,concat(round(`Q_单间夜_app_ld` / `C_单间夜_ld` *100, 2), '%') as `单间夜QC_ld`
        -- ,`Q_间夜量_ld`
        -- ,`Q_佣金_ld`
        -- ,`Q_订单量_ld`
        -- ,`Q_GMV_ld`
        ...
        -- ,`Q_中星间夜量_ld`
        -- ,`Q_低星间夜量_ld`
        -- ,`Q_佣金_ld` / `Q_GMV_ld` as `Q_收益率_ld`
        -- ,`Q_GMV_ld` / `Q_间夜量_ld` as `Q_ADR_ld`
        -- ,`Q_间夜量_ld` / `Q_订单量_ld` as `Q_单间夜_ld`
        ...
        -- ,`Q_中星间夜量_app_ld`
        -- ,`Q_低星间夜量_app_ld`
        -- ,`Q_佣金_app_ld` / `Q_GMV_app_ld` as `Q_收益率_app_ld`
        -- ,`Q_GMV_app_ld` / `Q_间夜量_app_ld` as `Q_ADR_app_ld`
        -- ,`Q_间夜量_app_ld` / `Q_订单量_app_ld`  as `Q_单间夜_app_ld`
        ...
        -- ,`C_中星间夜量_ld`
        -- ,`C_低星间夜量_ld`
        -- ,`C_佣金_ld` / `C_GMV_ld` as `C_收益率_ld`
        -- ,`C_GMV_ld` / `C_间夜量_ld` as `C_ADR_ld`
        -- ,`C_间夜量_ld` / `C_订单量_ld` as `C_单间夜_ld`
...
-- --- 离店口径Q数据
-- left join order_info_app_checkout t3 
-- on t1.dt=t3.checkout_date and t1.mdd=t3.mdd and t1.user_type=t3.user_type
-- left join order_info_checkout t4
-- on t1.dt=t4.checkout_date and t1.mdd=t4.mdd and t1.user_type=t4.user_type
-- --- 离店口径C数据
-- left join c_order_info_checkout t5
-- on t1.dt=t5.checkout_date and t1.mdd=t5.mdd and t1.user_type=t5.user_type
