...
(
    select distinct 
           concat(substr(dt,1,4),'-',substr(dt,5,2),'-',substr(dt,7,2)) `日期`
           ,uv.user_name 
           ,'内容交叉'`渠道` 
           ,2 as user_number
    --酒店帖
    ...
    join(
        ...
        where dt = '%(DATE)s'  and tag_id in ('857', '860') and status = 0
    )c 
    on a.global_key = c.global_key
    left join 
    (
    ...
    (
        ...
        where  dt >= '20250820' and dt <= '%(DATE)s'
    )d 
    on a.global_key=d.global_key
    left join uv on d.user_id=uv.user_name and concat(substr(dt,1,4),'-',substr(dt,5,2),'-',substr(dt,7,2)) =uv.`日期`
...
(
    ...
    on d.user_name=uv.user_name 
    where d.`日期` >=date_sub(uv.`日期`,7) and d.`日期` <= uv.`日期` and d.user_name is not null
)           
,user_gnjd as ----国内酒店
(
    select distinct `日期`
    ...
    left join(
            ...
            where dt=date_sub(current_date,1) and  order_date>='2022-11-01'
            )g
    on uv.user_id = g.user_id
    where  g.order_date>=date_sub(uv.`日期`,365) and g.order_date<=uv.`日期` and g.user_id is not null
)               
, user_channel  ---流量来源渠道整理
as (
    select
    distinct `日期` ,user_name ,`渠道`
    from(
        select `日期`,user_name ,`渠道`
            ,row_number() over(partition by `日期`,user_name order by user_number) as user_level 
        from(   select `日期`,user_name ,`渠道`,user_number from user_jc 
                union all
                select `日期`,user_name ,`渠道`,user_number from user_xhs 
                union all 
                select `日期`,user_name ,`渠道`,user_number from user_nr
                union all
                select `日期`,user_name ,`渠道`,user_number from user_hd 
                union all 
                select `日期`,user_name ,`渠道`,user_number from user_gnjd
            )t 
        )tt 
    where user_level = 1 
)
...
(
    select order_date 
        ...
        ,count(distinct case when coupon_id is not null and batch_series not in ('MacaoDisco_ZK_5e27de','2night_ZK_952825','3night_ZK_ad8c83') and batch_series not like '%23base_ZK_728810%' and batch_series not like '%23extra_ZK_ce6f99%' then order_no else null end) as `Q_用券订单量`
        ,count(distinct case when coupon_id is not null and batch_series not in ('MacaoDisco_ZK_5e27de','2night_ZK_952825','3night_ZK_ad8c83') and batch_series not like '%23base_ZK_728810%' and batch_series not like '%23extra_ZK_ce6f99%' and order_date = b.min_order_date then order_no else null end ) as `新客用券订单量`
...
select `日期`,`渠道`,`流量占比`,UV,`CR`,`用券订单占比`,`订单量`,`间夜量`,`新客UV`,`新客流量占比`,`新客CR`,`新客用券订单占比`,`新客订单量`,`新客间夜量`
from
(
    select  order_date `日期`
        ,o.`流量来源` `渠道`
    ...
    left join 
    (
        select dates,`流量来源` ,sum(`UV`) `UV`  ,sum(`新客UV`) `新客UV`
        from
        (
            select dates
                ,mdd
    ...
    on  u.dates=o.order_date  and u.`流量来源`=o. `流量来源`
    left join
    (  
        select dates
            ,sum(`总UV`) `总UV`  
