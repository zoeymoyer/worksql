...
"""
1. 各组增加返现前佣金率、平均返现比率，佣金率使用包含转单和底价优势的。观测ADR、转单、底价优势这些在各个组分布情况
2. 由于C组是不固定挽留深度，需要拆开看不同深度分段的GMV、佣金、补贴金额、挽留成功率和ROI表现，以观测在深度上下限设置上是否存在优化空间
"""
--- 5、最新取消挽留实验数据-20260512
...
,abtest AS (--- 实验明细
    ...
    from ihotel_default.dw_ihotel_abtest_index_di_v2 a --user_id
    left join pub.dim_user_profile_nd b on a.ab_exp_value = b.user_id
    where a.dt between '2026-04-23'  and date_sub(current_date, 1)
        and type = 'flow'
        and user_id_type = 'user_id' 
        and ab_exp_id = '251210_ho_gj_qxwl'
    group by 1,2,3,4
...
,cancel_page AS ( --- O页取消页
    select concat(substr(dt, 1, 4),'-',substr(dt, 5, 2),'-',substr(dt, 7, 2)) AS dt
         ...
         ,get_json_object(value, '$.common.traceId') as trace_id
    from default.dw_qav_ihotel_track_info_di
    where  dt between replace('2026-04-23' ,'-','') and '%(DATE)s'
        and key = 'ihotel/OrderDetail/OrderInfo/click/actionBtn'
        and get_json_object(value, '$.ext.button.menu') = '取消订单'
...
,q_cashback as (--- 挽留成功订单-离店：有返现金额且已离店
    select *
          ...
          ,case when cb is null then '0未返现' 
                when cb / init_gmv <  0.05 then '1挽留深度[0-5%)' 
                when cb / init_gmv >= 0.05 and cb / init_gmv < 0.1  then '2挽留深度[5-10%)'
                when cb / init_gmv >= 0.1  and cb / init_gmv < 0.15 then '3挽留深度[10-15%)'
                when cb / init_gmv >= 0.15 and cb / init_gmv < 0.2  then '4挽留深度[15-20%)'
                else '5挽留深度(20%+]' end as cb_type
    from (
        select substr(cast(ext_flag_map['cancel_red_packet_join_activity_time'] as string), 1, 8) draw_date  ---格式20260203
            ,user_name
            ,order_no
...
from (
    select a.dt
        ,b.version
        ,nvl(a.yj_type, 'ALL') as yj_type
        ,nvl(a.cb_type, 'ALL') as cb_type,
        ...
        sum(case when e.user_name is not null then a.init_gmv end)  as `挽留成功GMV(预定)`,
        sum(case when e.user_name is not null then a.fx end)  as `挽留成功返现金额(预定)`,
        sum(case when e.user_name is not null then a.init_gmv end) / sum(case when e.user_name is not null then a.room_night end) as `挽留成功ADR(预定)`,
        ...
        sum(case when e.user_name is not null and is_bxt='Y' then a.fx end)  as `挽留成功变现提返现金额(预定)`,
        ...
        sum(case when g.order_no is not null  then a.fx end)  as `挽留成功捕鱼网返现金额(预定)`,
        sum(case when g.order_no is not null  then g.zdsy end)  as `挽留成功捕鱼网收益(预定)`,
        ...
        sum(f.yj) `挽留成功佣金(离店)`,
        sum(f.init_gmv) `挽留成功GMV(离店)`,
        sum(f.cb) `挽留成功返现金额(离店)`,
        count(distinct case when f.order_no is not null and g.order_no is not null then a.orderNo end)  as `挽留成功捕鱼网订单量(离店)`,
        sum(case when f.order_no is not null and g.order_no  is not null   then a.room_night end)  as `挽留成功捕鱼网间夜量(离店)`,
        sum(case when f.order_no is not null and g.order_no  is not null  then a.yj end)  as `挽留成功捕鱼网佣金(离店)`,
        sum(case when f.order_no is not null and g.order_no  is not null  then a.init_gmv end)  as `挽留成功捕鱼网GMV(离店)`,
        sum(case when f.order_no is not null and g.order_no  is not null  then a.fx end)  as `挽留成功捕鱼网返现金额(离店)`,
        sum(case when f.order_no is not null and g.order_no  is not null  then g.zdsy end)  as `挽留成功捕鱼网收益(离店)`,
    ...
    from (
        select t1.*,is_jp,order_no,room_night,init_gmv, yj,fx,yj_type,bp_realized,cb_type
        from  cancel_page t1 
...
,abtest AS (--- 实验明细
    ...
    from ihotel_default.dw_ihotel_abtest_index_di_v2 a --user_id
    left join pub.dim_user_profile_nd b on a.ab_exp_value = b.user_id
    where a.dt between '2026-05-13'  and date_sub(current_date, 1)
        and type = 'flow'
        and user_id_type = 'user_id' 
        and ab_exp_id = '251210_ho_gj_qxwl'
    group by 1,2,3,4
...
,cancel_page AS ( --- O页取消页
    select concat(substr(dt, 1, 4),'-',substr(dt, 5, 2),'-',substr(dt, 7, 2)) AS dt
         ...
         ,get_json_object(value, '$.common.traceId') as trace_id
    from default.dw_qav_ihotel_track_info_di
    where  dt between replace('2026-05-13' ,'-','') and '%(DATE)s'
        and key = 'ihotel/OrderDetail/OrderInfo/click/actionBtn'
        and get_json_object(value, '$.ext.button.menu') = '取消订单'
        and lower(device_id) not in (select orig_device_id from biguser) --- 新逻辑大单用户过滤
    group by 1,2,3,4
...
,cancelOrder AS (--- 取消订单:预定在26.5.13之后的
    select  order_no,
            DATE(first_cancelled_time) AS cancelDate,
            user_id,
            user_name,
    ...
    where dt = '%(DATE)s'
        ...
        and DATE(first_cancelled_time) >= '2026-02-03'
        and DATE(first_cancelled_time) <= date_sub(current_date, 1)
        and order_date >= '2026-05-13' and order_date <= date_sub(current_date, 1)
)
,q_cashback as (--- 挽留成功订单-离店：有返现金额且已离店且预定在26.5.13之后的
    select *
          ,case when yj / init_gmv  < 0 then '0负佣'  
            when yj / init_gmv  >= 0 and yj / init_gmv < 0.03 then '1低佣[0-3%]' 
            when yj / init_gmv  >= 0.03 and yj / init_gmv < 0.1 then '2中佣(3-10%]'
...
,order_all as (---所有订单
    ...
    from (
        select order_no,user_name,room_night,init_gmv
            ...
            ,coalesce(get_json_object(extendinfomap,'$.bp_adv_amount_realized'),0) as bp_realized --实际变现底价优势金额（间夜均） 变现提
        from default.mdw_order_v3_international
        where dt = '%(DATE)s' and order_date >= '2026-05-13' and order_date <= date_sub(current_date, 1)
    )
)
,q_order as (---- 大盘预定订单量
    select order_date
...
from (
    select a.dt
        ...
        count(distinct case when e.user_name is not null and d.user_name is not null then a.orderNo end)  as `挽留成功订单量(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null then a.room_night end)  as `挽留成功间夜量(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null then a.yj end)  as `挽留成功佣金(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null then a.init_gmv end)  as `挽留成功GMV(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null then a.fx end)  as `挽留成功返现金额(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null then a.init_gmv end) / sum(case when e.user_name is not null and d.user_name is not null then a.room_night end) as `挽留成功ADR(预定)`,

        count(distinct case when e.user_name is not null and d.user_name is not null and is_bxt='Y' then a.orderNo end)  as `挽留成功变现提订单量(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null and is_bxt='Y' then a.room_night end)  as `挽留成功变现提间夜量(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null and is_bxt='Y' then a.yj end)  as `挽留成功变现提佣金(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null and is_bxt='Y' then a.init_gmv end)  as `挽留成功变现提GMV(预定)`,
        sum(case when e.user_name is not null and d.user_name is not null and is_bxt='Y' then a.fx end)  as `挽留成功变现提返现金额(预定)`,
        ...
        count(distinct case when d.user_name is not null then c.order_no end) as `取消订单量`,

        count(distinct case when d.user_name is not null then f.order_no end) as `挽留成功订单量(离店)`,
        sum(case when d.user_name is not null then f.room_night else 0 end) as `挽留成功间夜量(离店)`,
        sum(case when d.user_name is not null then f.yj else 0 end) as `挽留成功佣金(离店)`,
        sum(case when d.user_name is not null then f.init_gmv else 0 end) as `挽留成功GMV(离店)`,
        sum(case when d.user_name is not null then f.cb else 0 end) as `挽留成功返现金额(离店)`,
...
with cancel_page AS ( --- O页取消页
    select concat(substr(dt, 1, 4),'-',substr(dt, 5, 2),'-',substr(dt, 7, 2)) AS dt
         ...
         ,get_json_object(value, '$.common.traceId') as trace_id
    from default.dw_qav_ihotel_track_info_di
    where  dt between replace('2025-11-01' ,'-','') and '%(DATE)s'
        and key = 'ihotel/OrderDetail/OrderInfo/click/actionBtn'
        and get_json_object(value, '$.ext.button.menu') = '取消订单'
    group by 1,2,3,4
)
