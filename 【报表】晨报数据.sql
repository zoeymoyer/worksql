...
, c_uv as 
    (select dt as `日期`
        , case 
            when provincename in ('澳门','香港') then provincename 
    ...
    where device_chl='app'
        and dt between date_sub(current_date,15) and date_sub(current_date,1)
    group by 1,2,3
    )        
...
, q_app_order as 
    (select order_date as `日期`
        ...
        , sum(case when a.batch_series in ('MacaoDisco_ZK_5e27de','2night_ZK_952825','3night_ZK_ad8c83') then (init_commission_after+nvl(coupon_substract_summary ,0)) 
...
, q_daily_order as 
    (select order_date1 as `日期`
        ...
        , case when order_date = b.min_order_date then '新客' else '老客' end as user_type 
        , sum(room_fee_corp) as `Q_GMV（C平台）`
        , sum(commission_corp+commission_map['settle_base_price_diff']) as `Q_佣金（C平台）`
    from 
        (select *
...
select `日期`
    ...
    , concat(round(((sum(`Q_佣金（C平台）`)/sum(`Q_GMV（C平台）`))-(sum(`C_佣金`)/sum(`C_GMV`)))*100, 2), '%')`收益率QC差(C平台)`
...
from
    ...
    left join q_app_order c on a.`日期` = c.`日期` and a.`目的地` = c.`目的地` and a.user_type=c.user_type
    ...
    left join q_daily_order f on a.`日期` = f.`日期` and a.`目的地` = f.`目的地` and a.user_type=f.user_type
    ) a
group by 1, 2
order by a.`日期` desc
