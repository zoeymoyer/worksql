...
from (
    ...
    from (
        ...
        from (
            ...
            where dt between '2025-01-01' and date_sub(current_date, 1)
                and source = 'hotel'
        ...
        where match_adult != 'false'
            or match_adult is null
        group by 1
    ) a
) a
left join (
    ...
    from (
        ...
        from (
            ...
            where dt between regexp_replace('2025-01-01', '-', '') and regexp_replace(cast(date_sub(current_date, 1) as string), '-', '')
                and source = 'app_intl'
