...
,c_uv as (
    ...
    from (
        ...
        from (
            ...
            left join temp.temp_yiquny_zhang_ihotel_area_region_forever e
                on a.countryname = e.country_name
...
,q_sugar_order as ( --全量积分订单
    ...
    where dt = '%(DATE)s'
        ...
        and order_date >= '%(FORMAT_DATE_30)s'
...
,q_order as ( --Q间夜量，返后佣金，ADR
    ...
    from (
        select order_date
            ...
            ,a.order_no
            ,physical_room_name
            ,case when a.batch_series in ('MacaoDisco_ZK_5e27de','2night_ZK_952825','3night_ZK_ad8c83') then init_commission_after + nvl(coupon_substract_summary, 0)
                  when a.batch_series like '%23base_ZK_728810%' or a.batch_series like '%23extra_ZK_ce6f99%' then init_commission_after + nvl(split(coupon_info['23base_ZK_728810'], '_')[1], 0) + nvl(split(coupon_info['23extra_ZK_ce6f99'], '_')[1], 0) + nvl(ext_plat_certificate, 0)
            ...
            ,case when coupon_substract_summary is null
                        or batch_series in ('MacaoDisco_ZK_5e27de','2night_ZK_952825','3night_ZK_ad8c83')
    ...
    left join (
        select distinct uniq_id
            ...
            ,case when -chased_discount_price_diff / qunar_before_coupons_cashback_price > 0.2 then qunar_before_coupons_cashback_price * 0.2
                  else -chased_discount_price_diff
             end as bp_advantage_amount_limit20
            -- 卖价优势信息
            ,-pay_price_diff as sp_advantage_amount
        from default.dwd_hotel_cq_compare_price_result_intl_hi
        where dt between '%(DATE_30)s' and '%(DATE)s'
        ...
        on cq_compare.orderNum = order_intl.order_no
        and cq_compare.qunar_physical_room_name = order_intl.physical_room_name
    group by 1
)
,c_order as ( --C间夜量，返后佣金
    ...
    from (
        ...
        where dt = from_unixtime(unix_timestamp() - 86400, 'yyyy-MM-dd')
            ...
            and extend_info['sub_book_channel'] = 'Direct-Ctrip'
            and terminal_channel_type = 'app'
            and (extend_info['CANCEL_TIME'] is null
                or extend_info['CANCEL_TIME'] = 'NULL'
                or substr(extend_info['CANCEL_TIME'], 1, 10) > substr(order_date, 1, 10))
...
,cq_compare as (
    -- 用户视角支付价产品力分布情况
    select substr(crawl_time, 1, 10) as crawl_date
        --,`目的地`
