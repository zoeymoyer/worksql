...
,l2d_price_10min as (--- L2D在10min内变价
    ...
    from (
        ...
        and t2.action_time >= t1.action_time
...
,d2b_price_10min as (--- D2B在10min内变价
    ...
    from (
        ...
        join (
            ...
            -- )
        ) t2 on t1.booking_date=t2.booking_date and t1.device_id=t2.orig_device_id and t2.log_time >= t1.action_time
...
,b2o_price_10min as (--- B2O在10min内变价
    ...
    from (
        ...
        ) t2 on t1.booking_date=t2.booking_date and t1.orig_device_id=t2.orig_device_id and t2.log_time >= t1.log_time
        group by 1,2,3,4,5,6,7
    ) where ts <= 600
)
...
select t1.booking_date
      ,round(100 - `L2D-房态房价一致率_uv`,2) as `L2D-房态房价不一致率_uv`
      ,round(100 - `D2B-房态房价一致率_uv`,2) as `D2B-房态房价不一致率_uv`
      ,round(100 - `B2O-房态房价一致率_uv`,2) as `B2O-房态房价不一致率_uv`
      ,round(`整体不一致uv` / dau * 100, 2) as `整体不一致率_uv`

      ,round(100 - `L2D-房态房价一致率`,2) as `L2D-房态房价不一致率`
      ,round(100 - `D2B-房态房价一致率`,2) as `D2B-房态房价不一致率`
      ,round(100 - `B2O-房态房价一致率`,2) as `B2O-房态房价不一致率`
...
left join (
    ...
    from (---- 页面整体变价
        ...
        select concat(substr(a.dt,1,4),'-',substr(a.dt,5,2),'-',substr(a.dt,7,2)) as  booking_date,user_id,orig_device_id device_id
            ...
            from(
                select dt,log_time,q_trace_id,ret,err_code,err_message,err_sys,ischange,user_id,orig_device_id
                from view_dw_user_app_booking_qta_di 
                where  dt between '%(DATE_15)s' and '%(DATE)s'
                    and source='app_intl'
                    and platform in ('adr','ios')
...
,overall_price as (--- 整体变价不一致
    select booking_date
          ,count(distinct device_id) `整体不一致uv` 
          ,count(device_id) `整体不一致pv` 
          ,count(distinct case when is_ft='Y' then device_id end) `整体不一致uv_剔除房态` 
          ,count(case when is_ft='Y' then device_id end) `整体不一致pv_剔除房态` 

          ,count(distinct case when is_ft='Y' and ke='D2B' then device_id end) `整体不一致uv_剔除D2B房态` 
          ,count(case when is_ft='Y' and ke='D2B' then device_id end) `整体不一致pv_剔除D2B房态` 
    from (
        select booking_date,device_id
               ,case when ((listPrice is not null and listPrice not in (-1,0)) and (low_price is null or low_price =0 ))  then 'Y' else 'N' end  is_ft
               ,'L2D' ke
        from l2d_price_10min
        where  
            ((listPrice is not null and listPrice not in (-1,0)) and (low_price is null or low_price =0 ))   ---- 房态不一致
        ...
        select booking_date,orig_device_id device_id 
              ,case when (ret!='false' and ret is not null) then 'Y' else 'N' end is_ft
              ,'D2B' ke
        from d2b_price_10min
        where (
                (ischange='true' and ret='true') ---房价不一致
        ...
        select booking_date,orig_device_id device_id, 'N' is_ft,'B2O' ke
        from b2o_price_10min
        where (ret='false' or ret is null)
    )t group by 1
)
...
,overall_price_nonft as (--- 整体变价不一致剔除房态
    select booking_date
...
,overall_price_nond2bft as (--- 整体变价不一致剔除D2B房态
    select booking_date
...
left join overall_price t4 on t1.booking_date=t4.booking_date
left join gj_dau t5 on t1.booking_date=t5.dt
left join overall_price_nonft t6 on t1.booking_date=t6.booking_date
left join overall_price_nond2bft t7 on t1.booking_date=t7.booking_date
order by `日期` desc
;