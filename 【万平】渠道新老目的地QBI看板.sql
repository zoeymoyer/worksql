...
SELECT t1.dates 
        ...
        ,COALESCE(t1.uv / t3.uv_all, 0) as  uv_rate
        ,COALESCE(t2.order_no / t1.uv, 0)  as cr
        ,COALESCE(t2.q_order_no, 0) / COALESCE(t2.order_no, 0) as q_order_no_rate
        ,COALESCE(t2.init_gmv, 0)init_gmv
from q_uv_info t1 
left join order_info t2 
on t1.dates=t2.order_date and t1.mdd=t2.mdd and t1.chann=t2.channel and t1.user_type1=t2.user_type1
left join (  --- 计算流量占比
    select dates,mdd,uv as uv_all
    from q_uv_info 
