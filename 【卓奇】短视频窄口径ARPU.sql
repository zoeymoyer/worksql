...
,nu_data as (
    ...
    from uv t1
    join q_order t2 on t1.user_id=t2.user_id and t1.dt=t2.order_date
    where t1.user_type = '新客'
    group by 1,2
)
...
from (
    select t1.order_date
        ,count(distinct t1.user_id) uv
        ,sum(case when datediff(t2.order_date, t1.order_date) = 0    then ldyj end) yj0
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 1   then ldyj end) yj1
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 2   then ldyj end) yj2
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 3   then ldyj end) yj3
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 4   then ldyj end) yj4
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 5   then ldyj end) yj5
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 6   then ldyj end) yj6
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 7   then ldyj end) yj7
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 30  then ldyj end) yj30
        ,sum(case when datediff(t2.order_date, t1.order_date) <= 180 then ldyj end) yj180
    from (
        select t1.dt order_date,t1.user_id
        from video_data t1
    ) t1 
