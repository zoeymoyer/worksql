...
,q_order_app as (
    select order_date
            ...
            ,a.user_id,order_no,checkin_date,checkout_date
...
select t1.user_id,t1.user_type
        ,case when user_status='截止t-1未下单' and t2.user_id is not null then '其他' 
              when user_status='截止t-1未下单' and t2.user_id is null then '截止t-1未下单'
              else user_status end user_status
        ,order_mdd,order_checkin_date
        ,order_checkout_date,mdd,checkin_date,checkout_date
from (
    select *,row_number() over(partition by user_id order by user_status_num) rn 
    from (
        select t1.user_id
                ,t1.user_type
                ,case when t2.user_id is not null and t2.order_date = '2026-01-01' then '浏览当天下单'
                    when t2.user_id is not null 
                            and t2.order_date between '2025-10-01' and '2025-12-31' 
                            then '元旦前下单'
                    when t2.user_id is not null 
                            and t2.order_date between '2026-01-02' and '2026-01-07' 
                            then '浏览后7天内下单'
                    when t2.user_id is not null 
                            and t2.order_date between '2026-01-08' and date_sub(current_date, 1)
                            then '浏览7天后下单'
                    when t2.user_id is not null then '1号至浏览当天前下单'
                    else '截止t-1未下单' end user_status
                ,case when t2.user_id is not null and t2.order_date = '2026-01-01' then 1
                    when t2.user_id is not null 
                            and t2.order_date between '2025-10-01' and '2025-12-31' 
                            then 2
                    when t2.user_id is not null 
                    ...
                    when t2.user_id is not null 
                            and t2.order_date between '2026-01-08' and date_sub(current_date, 1)
                            then 4
                    when t2.user_id is not null then 5
                    else 6 end  user_status_num
                ,t2.mdd order_mdd
                ,t2.checkin_date order_checkin_date
                ,t2.checkout_date order_checkout_date
                ,t1.mdd,t1.checkin_date,t1.checkout_date
        from (
            select user_id,user_type,checkin_date,checkout_date,mdd,max(action_time) action_time
            from b_uv
            group by 1,2,3,4,5
        )t1 
        left join q_order_app t2 on t1.user_id=t2.user_id and t1.checkin_date=t2.checkin_date and t1.checkout_date=t2.checkout_date and t1.mdd=t2.mdd
    )t 
) t1 
left join (select user_id from q_order_app group by 1) t2 on t1.user_id=t2.user_id
where rn = 1
;
...
,c_order as (
    ...
    where dt = '%(FORMAT_DATE)s'
      ...
      and substr(order_date,1,10) >= '2025-10-01'
...
,b_uv as (
    ...
    where device_chl='app' and page_short_domain='dbo' and detail_dingclick_cnt> 0 
    and  dt= '2026-01-01'  
)
...
from (
    ...
    from (
        select t1.user_id
                ...
                ,t1.mdd,t1.checkin_date,t1.checkout_date
        from (
            select user_id,user_type,checkin_date,checkout_date,mdd
            from b_uv
            group by 1,2,3,4,5
        )t1 
        left join c_order t2 on t1.user_id=t2.user_id and t1.checkin_date=t2.checkin_date and t1.checkout_date=t2.checkout_date and t1.mdd=t2.mdd
    )t 
) t1 
left join (select user_id from c_order group by 1) t2 on t1.user_id=t2.user_id
where rn = 1
;
...
left join (---- C得DBO转化
    ...
    group by 1,2
)t2 on t1.dt=t2.dt  and t1.new_mdd=t2.new_mdd
order by 1 desc
; 
...
from(---- Q得DBO转化
    select 
        ...
        ,count(distinct b.user_id) order_user_cnt
    from  uv a  -- 流量表
    left join q_app_order b on a.dt=b.order_date and a.user_id=b.user_id  and a.user_type=b.user_type  -- 订单表
    group by 1,2
)t1   
left join (---- C得DBO转化
    select t1.dt
        ...
        ,count(distinct t2.ubt_user_id) as order_user_cnt_c
    from c_uv t1 
    left join c_order t2 on t1.dt=t2.dt and t1.uid=t2.ubt_user_id and t1.user_type=t2.user_type
    group by 1,2
...
from(
    ...
    from (
        select 
            ...
            ,count(distinct b.user_id) order_user_cnt
        from  uv a  -- 流量表
        left join q_app_order b on a.dt=b.order_date and a.user_id=b.user_id and a.mdd=b.mdd and a.user_type=b.user_type  -- 订单表
        group by 1,2,3
    ) a 
    group by 1
)t1   
left join (
    ...
    from (
        select t1.dt
            ...
            ,count(distinct t2.ubt_user_id) as order_user_cnt_c
        from c_uv t1 
        left join c_order t2 on t1.dt=t2.dt and t1.uid=t2.ubt_user_id and t1.mdd=t2.mdd and t1.user_type=t2.user_type
        group by 1,2,3
    ) t 
    group by 1
)t2 on t1.dt=t2.dt 
