...
,q_uv_info as
(   ---- 流量汇总
    select '${zdt.addDay(-1).format("yyyy-MM-dd")}' dt
        ,count(user_id)   uv
    from uv
) 
...
,order_info_app as ( --- q app 订单汇总
    select '${zdt.addDay(-1).format("yyyy-MM-dd")}' order_date 
...
,q_data_info as (
    ...
    from q_uv_info t1 
    left join order_info t2 on t1.dt=t2.order_date
...
from c_uv_info t1 
left join c_order_info t2 on t1.dt=t2.dt 
)
...
/******************************** QC分区域分新老结果数据 ********************************/ 
select q_room_night_app / c_room_night  as qc_rn_rate -- 间夜QC
